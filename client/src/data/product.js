export const productsData = [
  {
    id: "led-handheld-stroboscope",
    slug: "led-handheld-stroboscope",
    name: "LED Handheld Model Stroboscope",
    shortDescription: "Rechargeable battery-operated inspection device used to check high-speed moving and rotating parts without stopping the machine.",
    externalLink: "https://www.imagetechindustries.com/products/led-handheld-model-stroboscope",
    images: [
      "/STROBOSCOPE/1 . LED HAND MODEL STROBOSCOPE MODEL - 1/1.jpg",
      "/STROBOSCOPE/1 . LED HAND MODEL STROBOSCOPE MODEL - 1/15.jpg",
      "/STROBOSCOPE/1 . LED HAND MODEL STROBOSCOPE MODEL - 1/16.jpg",
      "/STROBOSCOPE/1 . LED HAND MODEL STROBOSCOPE MODEL - 1/17.jpg",
      "/STROBOSCOPE/1 . LED HAND MODEL STROBOSCOPE MODEL - 1/18.jpg"
    ],
    overview: "LED Handheld Model Stroboscope from ImageTech Industries is a highly advanced, portable inspection device designed specifically for checking high-speed moving and rotating parts. It creates a visual 'freeze-frame' effect to let you observe machine movement safely without ever stopping production.",
    detailedDescription: `In modern manufacturing and printing, stopping a machine to check for quality issues costs time and money. This stroboscope solves that problem by using powerful LED flashing lights to create a visual "freeze-frame" effect. It makes fast-moving machinery appear as if it is standing perfectly still, allowing you to inspect every detail clearly without ever stopping production.

Why is this important for your business? When you use our LED Handheld Stroboscope, you can detect print registration errors, check web movement, and observe the condition of rollers and gears while the machine is running at full speed. This means you catch defects early, reduce material wastage, and maintain high production efficiency. The LED light source is incredibly bright, ensuring that even in well-lit factory environments, the flashing light provides excellent contrast for clear, stress-free viewing.

Portability is a key feature of this model. Because it is powered by a high-capacity rechargeable battery, there are no messy cords or wires to worry about. You can easily carry it across different production lines, from the printing press to the packaging area, making it an essential tool for maintenance teams and quality control inspectors. Its lightweight, ergonomic handheld design means you can use it for extended periods without fatigue.

Whether you are in the printing industry, packaging, coating, or any manufacturing sector that relies on high-speed rotating equipment, the LED Handheld Model Stroboscope is a simple yet powerful investment. It is built tough for industrial environments, ensuring years of reliable service. By giving your operators the ability to perform visual inspections safely and accurately during operation, you are directly improving your product quality and your bottom line.`,
    keyFeatures: [
      "Handheld and portable ergonomic design for easy carrying",
      "High-intensity LED light source for bright, clear inspection",
      "Rechargeable battery operation eliminates the need for power cords",
      "Allows visual inspection without stopping the machine, saving time",
      "Perfect for checking high-speed rotating parts and rollers",
      "Ideal for printing machine inspection and web movement checking",
      "Long-lasting battery life for uninterrupted factory floor use",
      "Simple and easy to operate with minimal training required",
      "Essential tool for preventative maintenance and strict quality control"
    ],
    applications: "The LED Handheld Model Stroboscope is extensively used for inspecting printing machines, rollers, rotating components, and other high-speed moving parts. It is highly effective in rotogravure and flexographic printing, packaging, converting, coating, laminating, textile processing, and general industrial maintenance where visual inspection during active machine operation is absolutely critical.",
    specifications: [
      { label: "Product Type", value: "LED Handheld Stroboscope" },
      { label: "Light Source", value: "LED" },
      { label: "Operation", value: "Battery Operated" },
      { label: "Power", value: "Rechargeable Battery" },
      { label: "Design", value: "Handheld & Portable" },
      { label: "Application", value: "Machine & Rotating Parts Inspection" },
      { label: "Suitable For", value: "Printing, Packaging & Industrial Applications" }
    ],
    faqs: [
      { question: "What is a handheld stroboscope used for?", answer: "It is used to visually freeze the motion of high-speed moving parts, allowing operators to inspect machinery for faults and quality issues without stopping production." },
      { question: "Is this stroboscope battery operated?", answer: "Yes, it features a built-in rechargeable battery, making it completely portable and free from power cords." },
      { question: "What type of light does this stroboscope use?", answer: "It utilizes a high-intensity LED light source that produces very bright, crisp flashes." },
      { question: "Can it be used for printing machine inspection?", answer: "Absolutely. It is highly recommended for checking print registration, color alignment, and web movement on fast-running printing presses." },
      { question: "Can the machine be inspected without stopping production?", answer: "Yes! That is the primary benefit of the stroboscope—it creates an optical illusion that freezes motion, allowing safe inspection while the machine runs at full speed." },
      { question: "Is the stroboscope portable and easy to carry?", answer: "Yes, its lightweight, handheld, and battery-operated design makes it highly portable across the factory floor." },
      { question: "Where can this stroboscope be used?", answer: "It can be used in printing, packaging, converting, coating, laminating, and general industrial maintenance applications." }
    ],
    metaTitle: "LED Stroboscope Light | Handheld RPM Strobe Price in India",
    metaDescription: "Buy high-performance LED stroboscope light at best price in India. Digital handheld strobe instrument for RPM measurement, tachometer speed testing & printing inspection."
  },
  {
    id: "led-handheld-stroboscope-lens",
    slug: "led-handheld-stroboscope-with-lens",
    name: "LED Handheld Model Stroboscope with Lens",
    shortDescription: "Rechargeable battery-operated inspection device equipped with a built-in magnifying lens for focused, high-precision checking.",
    externalLink: "https://www.imagetechindustries.com/products/led-handheld-model-stroboscope-with-lens",
    images: [
      "/STROBOSCOPE/2 . LED HAND HELD STROBOSCOPE MODEL  2/11.jpg",
      "/STROBOSCOPE/2 . LED HAND HELD STROBOSCOPE MODEL  2/12.jpg",
      "/STROBOSCOPE/2 . LED HAND HELD STROBOSCOPE MODEL  2/13.jpg",
      "/STROBOSCOPE/2 . LED HAND HELD STROBOSCOPE MODEL  2/9.jpg",
      "/STROBOSCOPE/2 . LED HAND HELD STROBOSCOPE MODEL  2/Product images .jpg"
    ],
    overview: "LED Handheld Model Stroboscope with Lens is an advanced portable inspection device featuring a powerful LED flashing light combined with an integrated magnifying lens. It provides an exceptionally clear and focused view of moving machine components during critical inspections without stopping the machine.",
    detailedDescription: `While standard stroboscopes are great for general inspection, there are times when you need to look very closely at tiny, intricate details. This advanced portable device features a powerful LED flashing light combined with an integrated magnifying lens. This lens focuses the light and magnifies the viewing area, helping you achieve an exceptionally clear and focused view of moving machine components.

In industries like high-resolution printing, micro-electronics, or fine coating, even the smallest error can lead to huge losses. By using this stroboscope, operators can observe detailed machine movement and verify production conditions with pinpoint accuracy—all without stopping the machine. The focused LED beam ensures that tiny defects, micro-registration shifts, and fine gear wear are easily spotted while the machinery is fully operational. 

Convenience and ease of use are at the heart of this design. The stroboscope operates on a high-capacity rechargeable battery, making it completely portable. You don't need to drag power cables across the factory floor. Its handheld, ergonomic construction allows quality control inspectors and maintenance teams to effortlessly carry the unit and inspect multiple machines across various production areas throughout the day.

If you are looking for the ultimate tool to ensure flawless quality control in your manufacturing process, this is it. The LED Handheld Model Stroboscope with Lens is a practical, reliable, and highly effective instrument that empowers your team to maintain the highest industrial standards, reduce machine downtime, and consistently deliver perfect products to your customers.`,
    keyFeatures: [
      "Built-in magnifying lens for highly focused, precise inspection",
      "Intense LED light source for bright, clear viewing of small details",
      "Handheld and portable design for maximum convenience",
      "Rechargeable battery operation for cord-free factory floor use",
      "Perfect for inspecting high-speed moving parts and small rotating components",
      "Highly useful for detailed printing machine inspection and defect detection",
      "Allows thorough inspection while the machine is running at full speed",
      "Easy to carry and handle during long shifts",
      "Essential for preventative maintenance and high-precision quality checking"
    ],
    applications: "The LED Handheld Model Stroboscope with Lens is widely utilized for inspecting printing presses, fine rollers, small rotating components, and other high-speed moving parts that require a closer, magnified look. It is perfectly suited for high-resolution printing, packaging, label manufacturing, converting, coating, laminating, and other precision industrial applications.",
    specifications: [
      { label: "Product Type", value: "LED Handheld Stroboscope with Lens" },
      { label: "Light Source", value: "LED" },
      { label: "Lens", value: "Included (Built-in Magnification)" },
      { label: "Operation", value: "Battery Operated" },
      { label: "Battery", value: "Rechargeable" },
      { label: "Design", value: "Handheld & Portable" },
      { label: "Application", value: "Machine & Rotating Parts Inspection" },
      { label: "Suitable For", value: "Printing, Packaging & Industrial Applications" }
    ],
    faqs: [
      { question: "What is an LED Handheld Stroboscope with Lens used for?", answer: "It is used for the precise visual inspection of high-speed moving parts. The built-in lens provides a focused, magnified view of the inspection area, making it easier to spot tiny defects." },
      { question: "Does this stroboscope have a lens?", answer: "Yes, it features a built-in optical lens specifically designed for focused, magnified visual inspection." },
      { question: "Is the stroboscope rechargeable?", answer: "Yes, it is fully battery-operated and rechargeable for maximum portability and convenience." },
      { question: "Can it be used for printing machine inspection?", answer: "Absolutely. The magnifying lens makes it especially useful for checking fine details like print registration, dot quality, and color alignment." },
      { question: "Is this stroboscope portable?", answer: "Yes, its compact, handheld, and battery-powered design ensures it is very easy to carry around the production floor." },
      { question: "Where can this stroboscope be used?", answer: "It is widely used in printing, packaging, label making, converting, and other industrial environments requiring high-precision inspection." }
    ],
    metaTitle: "LED Stroboscope with Lens | Precision Inspection Instrument Price",
    metaDescription: "Precision LED stroboscope instrument with built-in magnifying lens. Industrial uses for print registration & rotating machinery inspection. Check specs & price in India."
  },
  {
    id: "xenon-flash-tube-stroboscope",
    slug: "xenon-flash-tube-hand-held-stroboscope",
    name: "Xenon Flash Tube Hand Held Stroboscope",
    shortDescription: "Portable inspection device using Xenon flash technology to produce extremely short, intense flashes for blur-free viewing.",
    externalLink: "https://www.imagetechindustries.com/products/xenon-flash-tube-hand-held-stroboscope",
    images: [
      "/STROBOSCOPE/3 . U TUBE HAND MODEL STROBOSCOPE/14.jpg",
      "/STROBOSCOPE/3 . U TUBE HAND MODEL STROBOSCOPE/2.jpg",
      "/STROBOSCOPE/3 . U TUBE HAND MODEL STROBOSCOPE/3.jpg",
      "/STROBOSCOPE/3 . U TUBE HAND MODEL STROBOSCOPE/5.jpg"
    ],
    overview: "Xenon Flash Tube Hand Held Stroboscope is a premium, heavy-duty portable inspection instrument designed for the most demanding industrial environments. It uses advanced Xenon flash tube technology to produce rapid, high-intensity flashes with an extremely short duration, effectively 'freezing' motion for crystal-clear inspection.",
    detailedDescription: `When machinery operates at incredibly high speeds, standard lighting often isn't enough to capture a clear image. This stroboscope solves that problem by using advanced Xenon flash tube technology, which produces rapid, high-intensity flashes of light with an extremely short duration. This short flash duration effectively "freezes" motion without any blur, helping users visually inspect moving components with absolute, crystal-clear clarity.

This handheld stroboscope is an invaluable tool for checking rotating rollers, internal machine components, continuously moving webs, and other extremely fast-moving parts. The ultra-short flashing light creates a crisp, sharp visual effect that makes even the fastest movement easy to observe and analyze. By spotting misalignments, vibrations, or print errors early, you can make adjustments on the fly without halting the production line, saving both time and raw materials.

Designed with usability in mind, its handheld construction makes it incredibly convenient for maintenance teams, machine operators, and quality control personnel. You can easily move around the machine and inspect different areas dynamically. Whether you are troubleshooting a mechanical issue or ensuring the quality of a high-speed printing run, this stroboscope gives you the visual feedback you need instantly.

Widely used in printing, packaging, converting, and heavy industrial applications, the Xenon Flash Tube Hand Held Stroboscope is a trusted choice for professionals. If you need a reliable, high-performance tool for inspecting machinery movement during operation, this Xenon stroboscope delivers unmatched clarity and ease of use.`,
    keyFeatures: [
      "High-intensity Xenon Flash Tube technology for maximum brightness",
      "Ultra-short flash duration ensures zero-blur motion freezing",
      "Hand Held Construction allows for dynamic, flexible use around machines",
      "Portable Inspection Instrument that is easy to carry and handle",
      "Perfectly suited for inspecting High-Speed Moving Parts",
      "Highly useful for Rotating Component and Moving Web Inspection",
      "Allows thorough inspection without ever stopping the machine",
      "Essential for Printing Machine Inspection and color registration",
      "A must-have tool for Preventative Maintenance & Quality Control"
    ],
    applications: "The Xenon Flash Tube Hand Held Stroboscope is widely used for inspecting printing machines, fast-spinning rollers, rotating components, moving webs, and other high-speed machine parts. It is suitable for printing, packaging, converting, coating, laminating, paper manufacturing, and general heavy industrial high-speed inspection applications.",
    specifications: [
      { label: "Product Type", value: "Xenon Flash Tube Hand Held Stroboscope" },
      { label: "Light Source", value: "Xenon Flash Tube" },
      { label: "Design", value: "Hand Held & Portable" },
      { label: "Operation", value: "Flash Illumination" },
      { label: "Application", value: "Machine & Rotating Parts Inspection" },
      { label: "Suitable For", value: "Printing, Packaging & Industrial Applications" },
      { label: "Inspection Type", value: "Visual Inspection of Moving & Rotating Parts" }
    ],
    faqs: [
      { question: "What is a Xenon Flash Tube Hand Held Stroboscope?", answer: "It is a portable inspection device that uses a specialized xenon gas flash tube to produce ultra-short, extremely bright flashes of light. This visually freezes high-speed moving parts for easy inspection." },
      { question: "Why choose Xenon over other light sources?", answer: "Xenon flash tubes produce a much shorter flash duration. This extremely brief burst of light is better at freezing very high-speed motion without any motion blur, resulting in a crisper image." },
      { question: "Where is this stroboscope used?", answer: "It is used in fast-paced industrial environments like printing, packaging, paper manufacturing, and converting to inspect rotating parts, rollers, and moving webs." },
      { question: "Can it be used while the machine is running?", answer: "Yes, its primary purpose is to allow operators to inspect machinery safely and clearly while the machine is operating at full speed." },
      { question: "Is the stroboscope portable?", answer: "Yes, it features a handheld and portable design for easy movement around the factory floor." },
      { question: "Is it useful for printing machine inspection?", answer: "Yes, it is highly effective for inspecting print registration, color overlap, and web movement on high-speed presses." },
      { question: "Who can use a hand held stroboscope?", answer: "It is designed to be easily used by machine operators, maintenance engineers, and quality control personnel." }
    ],
    metaTitle: "Xenon Stroboscope Light | Flash Tube Bulb & Instrument Price",
    metaDescription: "Portable Xenon stroboscope light with replaceable high-intensity flash tube bulb. Learn working principle, motion freeze uses & get best manufacturer price in India."
  },
  {
    id: "u-tube-fixed-stroboscope",
    slug: "u-tube-fixed-model-stroboscope",
    name: "U Tube Fixed Model Stroboscope",
    shortDescription: "Machine-mounted inspection system designed for continuous, automated web inspection and synchronization.",
    externalLink: "https://www.imagetechindustries.com/products/u-tube-fixed-model-stroboscope",
    images: [
      "/STROBOSCOPE/4. FIXED MODEL STROBOSCOPE/19.jpg",
      "/STROBOSCOPE/4. FIXED MODEL STROBOSCOPE/20.jpg",
      "/STROBOSCOPE/4. FIXED MODEL STROBOSCOPE/21.jpg",
      "/STROBOSCOPE/4. FIXED MODEL STROBOSCOPE/22.jpg",
      "/STROBOSCOPE/4. FIXED MODEL STROBOSCOPE/4.jpg",
      "/STROBOSCOPE/4. FIXED MODEL STROBOSCOPE/6.jpg",
      "/STROBOSCOPE/4. FIXED MODEL STROBOSCOPE/7.jpg",
      "/STROBOSCOPE/4. FIXED MODEL STROBOSCOPE/8.jpg"
    ],
    overview: "U Tube Fixed Model Stroboscope is a robust, machine-mounted stroboscope system designed exclusively for inspecting continuously moving webs and high-speed machine processes. It provides highly controlled, wide-area flashing illumination across the entire width of your material.",
    detailedDescription: `Unlike handheld models, this stroboscope is permanently installed on your machinery. It provides highly controlled, wide-area flashing illumination that spans the entire width of your material, helping operators clearly observe web movement, precise printing conditions, and other moving components during continuous 24/7 production runs.

This fixed model stroboscope is the ultimate solution for applications where continuous, hands-free visual inspection is required. Because the unit is installed directly onto the machine chassis, operators do not need to hold it. Instead, they can comfortably monitor moving web materials securely during printing and converting operations. It can be set to automatically synchronize with the speed of your machine, meaning the flash rate perfectly matches the production speed without any manual adjustments.

To ensure a perfect fit for your specific machinery, the U Tube Fixed Model Stroboscope is available in four specific size models: ITI-1300, ITI-1000, ITI-700, and ITI-250. These models correspond to different web width capacities (from 250 mm all the way up to 1300 mm). This makes it effortless to select the perfect model according to your specific machinery width requirements, ensuring edge-to-edge illumination.

If you operate wide-web flexographic presses, rotogravure machines, or continuous packaging lines, the U Tube Fixed Model Stroboscope is an essential upgrade. It automates your quality control process, reduces operator fatigue, and guarantees that every inch of your product can be inspected for defects in real-time, greatly enhancing your overall manufacturing quality.`,
    keyFeatures: [
      "Fixed Model Stroboscope designed for permanent machine installation",
      "U Tube Design provides wide-area, uniform illumination across the web",
      "Machine Mount Installation allows for completely hands-free operation",
      "Available in Four Models (ITI-1300, ITI-1000, ITI-700, ITI-250) to fit your machine",
      "Web Width Options support everything from narrow 250 mm to wide 1300 mm webs",
      "Auto and Manual Web Synchronization matches flash rate to machine speed",
      "Operates on standard 230 VAC - 50 Hz Input Supply",
      "Designed specifically for Continuous 24/7 Web Inspection",
      "Highly useful for wide-web Printing Machine Inspection",
      "Perfectly suitable for demanding Packaging and Converting Applications"
    ],
    applications: "U Tube Fixed Model Stroboscopes are the industry standard for printing, packaging, converting, and other wide-web-based production processes. They are permanently installed for observing moving webs, verifying continuous print quality, monitoring web alignment, and ensuring machine operation stability during continuous, high-speed production.",
    specifications: [
      { label: "Product Type", value: "U Tube Fixed Model Stroboscope" },
      { label: "Available Models", value: "ITI-1300, ITI-1000, ITI-700 & ITI-250" },
      { label: "Supported Web Widths", value: "1300 mm, 1000 mm, 700 mm & 250 mm" },
      { label: "Input Supply", value: "230 VAC - 50 Hz" },
      { label: "Web Synchronization", value: "Auto / Manual" },
      { label: "Installation Type", value: "Fixed Machine Mount" },
      { label: "Primary Application", value: "Continuous Web Inspection & Machine Monitoring" },
      { label: "Suitable Industries", value: "Printing, Packaging & Converting Machines" }
    ],
    faqs: [
      { question: "What is a U Tube Fixed Model Stroboscope?", answer: "It is a large, machine-mounted stroboscope designed for continuous, hands-free visual inspection of wide moving webs and fast production processes." },
      { question: "How many models are available?", answer: "It is available in four size models: ITI-1300, ITI-1000, ITI-700, and ITI-250." },
      { question: "What web widths are supported?", answer: "The models are designed to support maximum web widths of 1300 mm, 1000 mm, 700 mm, and 250 mm respectively." },
      { question: "Does the stroboscope support automatic synchronization?", answer: "Yes, it supports both Auto and Manual web synchronization. When set to Auto, it receives signals from the machine to automatically match the flash rate to the machine's exact speed." },
      { question: "How is this stroboscope installed?", answer: "It features a sturdy machine mount design, allowing it to be permanently bolted directly onto the chassis of the production machine." },
      { question: "What is the input power supply?", answer: "The system operates on a standard 230 VAC - 50 Hz industrial input supply." },
      { question: "Where is this fixed stroboscope used?", answer: "It is widely used in printing presses, packaging lines, coating facilities, and converting plants that require continuous, full-width web inspection." },
      { question: "Can I select the model according to my machine web width?", answer: "Yes, you should choose the model that best matches the width of your machine's web to ensure complete, edge-to-edge illumination." }
    ],
    metaTitle: "U Tube Stroboscope Light | Fixed Sensor Sync Web Inspection Price",
    metaDescription: "Machine-mounted U-tube stroboscope light instrument with auto sensor speed synchronization for printing & converting. Wide-web motion analysis at direct factory price."
  }
];
